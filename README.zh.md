...
处理结束且没有报告的待检查警告时，任务以 **完成** 结束。这不保证所有个人信息都已被检测和遮盖。只要有失败、跳过，或者保存了
没有任何检测区域的文件，结果就会变成 **需要检查** 并给出摘要。任务结束后，
可在活动区域点击 **文件结果…**，筛选需要关注或失败的项目，查看详情并打开
输入或输出文件夹。开始下一次任务时结果会重置；未开始处理的文件不会列出。

检查完整跟踪空缺区间后勾选，即可记录用户已检查。编辑遮罩会重置勾选；
检查记录与遮盖范围的自动验证是两回事。文件结果支持按问题类型筛选和重新检查／处理
所选输入。选择空缺可从对应帧开始检查，使用当前设置保存到新的输出文件夹，
不保留之前的编辑。

缩略图在后台生成，同时最多运行两个任务。基本磁盘日志仅包含不带文件名的诊断信息，
详细日志需要在设置中单独启用。更改立即生效，设置中也可打开和删除现有日志。
