...
報告された未確認の警告がなく処理を終えると**完了**になります。すべての個人情報の検出・マスクを保証するものではありません。失敗、スキップ、
...
追跡の空白区間全体を確認してチェックすると、ユーザー確認済みとして記録されます。
マスクの編集でチェックはリセットされ、確認記録はマスク範囲の自動検証とは別です。
ファイル別結果では問題の種類を絞り込み、選択した入力を再確認・再処理できます。
空白を選ぶとそのフレームから確認を開始し、現在の設定で新しい出力フォルダーに保存します。
以前の編集は保持されません。

サムネイルは最大2件ずつバックグラウンドで生成します。基本ディスクログにはファイル名を
含めず、詳細ログは設定で明示的に有効にします。変更は即時に適用され、設定から既存の
ログを開いたり削除したりできます。
