...
| 플랫폼 | 지원 환경 | 다운로드 |
| --- | --- | --- |
| Windows | Windows 10 이상, 64비트 | [Windows 다운로드](https://github.com/nyabi-gh/CloakFrame/releases/latest) |
| macOS | macOS 15 이상, Apple Silicon | [macOS 다운로드](https://github.com/nyabi-gh/CloakFrame/releases/latest) |
| Linux | x86_64 | [Linux 다운로드](https://github.com/nyabi-gh/CloakFrame/releases/latest) |
...
보고된 미확인 경고 없이 처리를 마치면 **완료**로 끝납니다. 이는 모든 개인정보가 검출·마스킹되었다는 보증은 아닙니다. 실패, 건너뜀, 검출 영역
...
추적 공백 전체를 검토한 뒤 체크하면 사용자 확인 완료로 기록됩니다. 마스크 편집은
체크를 초기화하며, 확인 여부와 실제 가려짐의 자동 검증은 별개입니다. 파일별 결과에서는
문제 유형별 필터와 **선택한 입력 재검토 / 재처리**를 제공합니다. 공백을 선택하면 해당
프레임에서 재검토를 시작하며, 현재 설정으로 새 출력 폴더에 저장합니다. 이전 편집은
유지되지 않습니다.

썸네일은 백그라운드에서 최대 2개씩 생성합니다. 기본 디스크 로그에는 파일명이 없는
진단 정보만 기록하며, 상세 로그는 설정에서 별도로 켜야 합니다. 로그 설정은 즉시
적용되고 설정에서 기존 로그를 열거나 삭제할 수 있습니다.
